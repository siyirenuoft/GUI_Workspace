...
class TimelineCanvas(FigureCanvas):
    ...
    def plot_custom_signal(self, customized_signal, start_time, stop_time):
        signal_data = np.array(customized_signal["data"])

        # Generate time array for the entire timeline
        total_points = 500
        t = np.linspace(0, self.app_reference.total_time, total_points)
        
        # Ensure y_data is initialized
        if not hasattr(self, 'y_data'):
            self.y_data = np.zeros_like(t)

        # Determine the start and stop indices based on the scaling factor
        start_index = int((start_time / self.app_reference.total_time) * total_points)
        stop_index = int((stop_time / self.app_reference.total_time) * total_points)
        
        # Rescale the signal data to fit within the time range
        signal_length = stop_index - start_index
        signal_time = np.linspace(0, 1, len(signal_data))
        scaled_signal_data = np.interp(np.linspace(0, 1, signal_length), signal_time, signal_data)
        
        # Insert the scaled signal into y_data
        self.y_data[start_index:stop_index] = scaled_signal_data
        
        # Plot the updated signal
        self.axes.clear()
        self.axes.plot(t, self.y_data, color='blue')
        self.axes.set_xlim(0, self.app_reference.total_time)
        self.draw()
    ...
    def dropEvent(self, event):
        ...
        # Initialize the customized signal to None
        customized_signal = None

        # Check if the dropped signal is a predefined type
        if signal_type in ["Sine", "Square", "Saw", "Triangle", "Chirp", "FM", "PWM", "Noise"]:
            dialog = OscillatorDialog(signal_type, self)
            if dialog.exec() == QDialog.DialogCode.Accepted:
                config = dialog.get_config()
                # Generate the customized signal with the user inputted parameters
                t = np.linspace(0, 1, 500)  # Assuming 500 points for the timeline canvas
                if signal_type == "Sine":
                    data = np.sin(2 * np.pi * config["frequency"] * t + config["rate"] * t).tolist()
                elif signal_type == "Square":
                    data = np.sign(np.sin(2 * np.pi * config["frequency"] * t)).tolist()
                elif signal_type == "Saw":
                    data = (2 * (t * config["frequency"] - np.floor(t * config["frequency"] + 0.5))).tolist()
                elif signal_type == "Triangle":
                    data = (2 * np.abs(2 * (t * config["frequency"] - np.floor(t * config["frequency"] + 0.5))) - 1).tolist()
                elif signal_type == "Chirp":
                    data = np.sin(2 * np.pi * (config["frequency"] * t + 0.5 * config["rate"] * t ** 2)).tolist()
                elif signal_type == "FM":
                    data = np.sin(2 * np.pi * (config["frequency"] * t + config["rate"] * np.sin(2 * np.pi * config["frequency"] * t))).tolist()
                elif signal_type == "PWM":
                    data = np.where(np.sin(2 * np.pi * config["frequency"] * t) >= 0, 1, -1).tolist()
                elif signal_type == "Noise":
                    data = np.random.normal(0, 1, len(t)).tolist()
                else:
                ...
                # Store the customized signal data
                customized_signal = {"data": data}
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                # Generate normalized time array (0 to 1) for the signal
                t = np.linspace(0, 1, 500)
            
                if signal_type == "Envelope":
                    data = (config["amplitude"] * np.sin(2 * np.pi * 5 * t)).tolist()
                elif signal_type == "Keyed Envelope":
                    data = (config["amplitude"] * np.sin(2 * np.pi * 5 * t) * np.exp(-3 * t)).tolist()
                elif signal_type == "ASR":
                    data = np.piecewise(t, [t < 0.3 * config["duration"], t >= 0.3 * config["duration"]],
                                        [lambda t: config["amplitude"] * (t / (0.3 * config["duration"])), config["amplitude"]]).tolist()
                elif signal_type == "ADSR":
                    data = np.piecewise(t, [t < 0.1 * config["duration"], t < 0.2 * config["duration"], t < 0.5 * config["duration"], t < 0.7 * config["duration"], t >= 0.7 * config["duration"]],
                                        [lambda t: config["amplitude"] * (t / (0.1 * config["duration"])),
                                        lambda t: config["amplitude"] * (1 - 5 * (t - 0.1 * config["duration"]) / config["duration"]),
                                        0.5 * config["amplitude"],
                                        lambda t: 0.5 * config["amplitude"] - 0.25 * config["amplitude"] * (t - 0.5 * config["duration"]) / config["duration"],
                                        0.25 * config["amplitude"]]).tolist()
                elif signal_type == "Exponential Decay":
                    data = (config["amplitude"] * np.exp(-5 * t / config["duration"])).tolist()
                elif signal_type == "PolyBezier":
                    data = (config["amplitude"] * (t ** 3 - 3 * t ** 2 + 3 * t)).tolist()
                elif signal_type == "Signal Envelope":
                    data = (config["amplitude"] * np.abs(np.sin(2 * np.pi * 3 * t))).tolist()
                else:
                    # Ensure 'data' is always initialized
                    data = np.zeros_like(t).tolist()
        ...
        # Only proceed if a customized signal was created
        if customized_signal:
            # Show the time input dialog to get the start and stop time
            ...
            # If new stop_time exceeds current total_time, update total_time
            if stop_time > self.app_reference.total_time:
                self.update_total_time_and_rescale(stop_time)

            # Plot the new signal
            self.plot_custom_signal(customized_signal, start_time, stop_time)

    def update_total_time_and_rescale(self, new_total_time):
        # Rescale existing signals if total_time is updated
        old_total_time = self.app_reference.total_time
        self.app_reference.total_time = new_total_time

        # Generate the new time array based on the updated total time
        total_points = 500
        t_new = np.linspace(0, new_total_time, total_points)

        # If we have existing data, rescale it
        if hasattr(self, 'y_data'):
            t_old = np.linspace(0, old_total_time, len(self.y_data))
            
            # Interpolate old data to fit the new total time
            self.y_data = np.interp(t_new, t_old, self.y_data)

        # Re-plot the rescaled data
        self.axes.clear()
        self.axes.plot(t_new, self.y_data, color='blue')
        self.axes.set_xlim(0, new_total_time)
        self.draw()
